...
export const AllItemsView: React.FC<AllItemsViewProps> = ({
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortBy === 'room') return a.roomName.localeCompare(b.roomName, 'hr');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Svi spremljeni predmeti</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Pregledaj, sortiraj i upravljaj svim zabilježenim stvarima u kućanstvu
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="lg:col-span-2 relative">
            ...
            <input
              ...
              placeholder="Filtriraj po nazivu, kutiji..."
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-hidden"
            >
              <option value="updated">Zadnje uređivano</option>
              <option value="name">Naziv (A - Ž)</option>
              <option value="room">Po prostoriji</option>
              <option value="quantity">Količina</option>
            </select>
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
          <span>Pronađeno <strong>{sorted.length}</strong> od ukupno {items.length} predmeta</span>
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300 p-6">
          <p className="text-sm font-semibold text-slate-700">Niti jedan predmet ne odgovara filterima</p>
          <p className="text-xs text-slate-500 mt-1">Pokušaj prilagoditi ili poništiti filtere.</p>
        </div>
      ) : displayMode === 'grid' ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold uppercase tracking-wider">
                <tr>
                  <th className="px-4 py-3">Predmet</th>
                  <th className="px-4 py-3">Lokacija i spremište</th>
                  <th className="px-4 py-3">Kategorija</th>
                  <th className="px-4 py-3">Količina</th>
