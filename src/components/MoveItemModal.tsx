...
export const MoveItemModal: React.FC<MoveItemModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onMove(
      item.id,
      targetRoomId,
      selectedRoom ? getRoomName(selectedRoom) : item.roomName,
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900 text-sm">{t.moveModal.title}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <span className="text-xs text-slate-500">{t.moveModal.currentLocation}</span>
            ...
            <div className="mt-1 flex items-center gap-1.5 text-xs text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md">
              ...
              <span>{item.roomName} → <strong>{item.container}</strong></span>
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-slate-100 space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                {t.moveModal.newSubLocationLabel}
              </label>
              <input
                type="text"
          ...
          <div className="pt-3 flex items-center justify-end gap-2 border-t border-slate-100">
            ...
            >
              {t.moveModal.cancelBtn}
            </button>
            <button
              type="submit"
              onClick={(e) => handleSubmit(e)}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-amber-600 hover:bg-amber-700 active:bg-amber-800 rounded-lg shadow-xs cursor-pointer"
            >
              <Check className="w-4 h-4" />
              {t.moveModal.confirmBtn}
            </button>
          </div>
        </form>
      </div>
