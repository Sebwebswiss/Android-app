  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900">{t.containers.title}</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white p-4 rounded-2xl border border-slate-200">
        ...
        >
          <option value="all">{t.containers.allRoomsFilter} ({rooms.length})</option>
          {rooms.map(r => (
      ...
      {filteredContainers.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300 p-6">
          ...
          <p className="text-sm font-semibold text-slate-700">{t.containers.noContainersFound}</p>
          <p className="text-xs text-slate-500 mt-1">{t.containers.noContainersDesc}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredContainers.map(c => {
            const roomObj = rooms.find(r => r.id === c.roomId);
            const localizedRoomName = roomObj ? getRoomName(roomObj) : c.roomName;
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="mt-3 space-y-1">
                    <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                      {t.locations.containersInRoomTitle} ({c.items.length}):
                    </span>
                    <ul className="space-y-1 mt-1">
                      ...
                      {c.items.length > 3 && (
                        <li className="text-[11px] text-slate-400 italic">
                          + {c.items.length - 3} {t.finder.itemCountPlural}...
                        </li>
                      )}
                    </ul>
                  </div>
                ...
                <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  <button
                    type="button"
                    onClick={() => onShowBoxLabel(c.container, localizedRoomName)}
                  ...
                  >
                    ...
                    {t.containers.boxLabelBtn}
                  </button>

                  <button
                    type="button"
                    onClick={() => onSelectContainer(c.container, localizedRoomName)}
